...
Have a Jenkins slave run the test
  ...
  Get Jenkins to connect to the slave
    ...
    Click 'Save'. It's now ready to be built; The first build takes a lit bit
Distributing test amoung multiple slaves
  Emulate a second slave compute by following the same steps as before except change steps:
    from:
  ...
  Click 'Save'. It's now ready to be built; The first build could take a bit
